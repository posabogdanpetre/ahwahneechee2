...
/**
 * getCoffeeDetails action handler.
 *
 * Accepts a coffee ID or name and returns the full product details.
 *
 * @param {object} params
 * @param {string} params.coffeeIdOrName - The coffee product ID (e.g. "hbdr212") or name.
 */
...
const COFFEE_LIST = [
    ...
    },
    {
        ...
        longDescription: 'Indulge in the rich and aromatic experience of our 100% Arabica beans, expertly infused with the delightful flavors of French vanilla and caramel. These premium coffee beans are carefully selected and roasted to perfection, ensuring a smooth and balanced taste with every sip. The subtle sweetness of French vanilla complements the rich, buttery notes of caramel, creating a harmonious blend that is both comforting and invigorating.',
        category: 'Coffee Pods'
    }
];

module.exports = async ({ coffeeIdOrName = '' }) => {
    const query = String(coffeeIdOrName).trim().toLowerCase();

    if (!query) {
        return {
            content: [{ type: 'text', text: 'Please provide a coffee ID or name.' }],
            structuredContent: { coffee: null }
        };
    }

    const coffee = COFFEE_LIST.find(
        (c) => c.id.toLowerCase() === query || c.name.toLowerCase() === query
    );

    if (!coffee) {
        return {
            content: [{ type: 'text', text: `No coffee found matching: ${coffeeIdOrName}` }],
            structuredContent: { coffee: null }
        };
    }
    ...
    return {
        content: [
            {
                type: 'text',
                text: `${coffee.name} (${coffee.sku}) — ${coffee.price}\n${coffee.longDescription}`
            }
        ],
        structuredContent: { coffee }
    };
};
